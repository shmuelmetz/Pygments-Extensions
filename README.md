...
- **NetRexx**: not a mainframe technology (it's JVM-based), so IBM-MAIN — right for PL/I because of that community's real mainframe weight — isn't the fit here. The correct venue is NetRexx's own home, [groups.io/g/netrexx](https://groups.io/g/netrexx/topics) (the NetRexx Forum, linked directly from netrexx.org), cross-posted to `main@rexxla-members.groups.io` — the same two-venue pattern already used for ooRexx, since NetRexx is likewise a RexxLA project. Draft prepared, not yet sent: `outreach-netrexx-testdata-draft.md` at the repo root, asking for real `.nrx` source to validate the lexer against (built independently from the Language Reference, not a fork of the classic-Rexx or ooRexx lexers here) and flagging the specific gaps still open — exception/select/signal grammar not yet fully cross-checked, and no real-world corpus pass yet, the same bar the ooRexx/PL/I lexers cleared before their own announcements.
