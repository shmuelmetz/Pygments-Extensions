...
- **ooRexx**: [RexxLA](https://www.rexxla.org/) (the Rexx Language Association) is the right organization — ooRexx is itself a RexxLA project — but RexxLA runs several distinct mailing lists (e.g. ARB, members) for different purposes, and which one is appropriate for this kind of announcement is not yet resolved. Don't guess: ask the user which list fits when it's actually time to draft the announcement. Useful both before and after a working lexer exists: real-world source samples for the test suite are worth asking for early; an announcement and request for review/testing once there's something concrete to show.
